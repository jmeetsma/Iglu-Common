...
import org.ijsberg.iglu.access.EntryPoint;
import org.ijsberg.iglu.access.Request;
import org.ijsberg.iglu.access.RequestManager;
import org.ijsberg.iglu.access.Session;
import org.ijsberg.iglu.access.StandardRequest;
import org.ijsberg.iglu.access.StandardSession;
 ...
 * The hashcode of a thread is used as identifier. This should be reliable
 * since the hashcode mechanism used is inherited from Object.
 * The hashCode method defined by class <tt>Object</tt> returns distinct
 * integers for distinct objects.
 */
public class StandardRequestManager implements RequestManager, Pageable
{
	//session storage
...
//	private Request initialRequest;
	...
	//classloader that is passed to to each and every client thread
...
//					boundRequest = new StandardRequest(threadId, entryPoint, this, (application.getAdminRealm() == entryPoint.getRealm()), internalRequests);
					boundRequest = new StandardRequest(this);
				}
/*				else
				{
	 ...
	 * @see this#bindInternalProcess(Realm,Thread,String)
	 */
	...
	{
		StandardSession session = new StandardSession(/*application,*/ sessionTimeout, defaultUserSettings);
		synchronized (sessions)
		{
			synchronized (sessionsMirror)
			{
